...
{
    public class SAMOCCTShellsDistance : GH_SAMVariableOutputParameterComponent
    {
        public override Guid ComponentGuid => new Guid("c9e09c3d-2b4f-4aaf-b1ed-4a3ebb59cf17");
        ...
        protected override GH_SAMParam[] Inputs
        {
            get
            {
                ...
                global::Grasshopper.Kernel.Parameters.Param_GenericObject shells = new global::Grasshopper.Kernel.Parameters.Param_GenericObject() { Name = "_shells", NickName = "_shells", Description = "First set of closed volumes. Accepts SAM Shells or closed Rhino Breps/polysurfaces that convert to SAM Shells.", Access = GH_ParamAccess.list };
                shells.DataMapping = GH_DataMapping.Flatten;
                result.Add(new GH_SAMParam(shells, ParamVisibility.Binding));

                global::Grasshopper.Kernel.Parameters.Param_GenericObject otherShells = new global::Grasshopper.Kernel.Parameters.Param_GenericObject() { Name = "_otherShells", NickName = "_otherShells", Description = "Second set of closed volumes to measure the distance to.", Access = GH_ParamAccess.list };
        ...
        protected override void SolveInstance(IGH_DataAccess dataAccess)
        {
            ...
            List<GH_ObjectWrapper> otherShellWrappers = new List<GH_ObjectWrapper>();
            index = Params.IndexOfInputParam("_otherShells");
            if (index == -1 || !dataAccess.GetDataList(index, otherShellWrappers) || otherShellWrappers == null)
            {
                AddRuntimeMessage(GH_RuntimeMessageLevel.Error, "Invalid other shells");
                return;
            }
            ...
            List<Shell> otherShells = new List<Shell>();
            foreach (GH_ObjectWrapper objectWrapper in otherShellWrappers)
            {
                if (Query.TryGetSAMGeometries(objectWrapper, out List<Shell> shells_Temp) && shells_Temp != null)
                {
                    otherShells.AddRange(shells_Temp);
                }
            }

            double? distance = Geometry.OCCT.Query.ShellsDistance(shells, otherShells, out OcctCellComplexResult result, out Point3D pointOnShells, out Point3D pointOnOtherShells, new OcctBuildOptions { Tolerance = tolerance, FuzzyTolerance = fuzzyTolerance });
            List<string> diagnostics = result?.Diagnostics?.Select(x => x.ToString()).ToList();

            index = Params.IndexOfOutputParam("Distance");
            if (index != -1 && distance.HasValue)
            {
                dataAccess.SetData(index, distance.Value);
            }

            index = Params.IndexOfOutputParam("PointOnShells");
            if (index != -1)
            {
                dataAccess.SetData(index, pointOnShells);
            }

            index = Params.IndexOfOutputParam("PointOnOtherShells");
            if (index != -1)
            {
                dataAccess.SetData(index, pointOnOtherShells);
            }
            ...
            if (index_Successful != -1)
            {
                dataAccess.SetData(index_Successful, result != null && result.Success && distance.HasValue);
            }
